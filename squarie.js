...
canvas.addEventListener('mousedown', function(e) {
    ...
    for (let i = 0; i < btnp.length; i++) {
        if (btnp[i][0] <= x && btnp[i][2] >= x && btnp[i][1] <= y && btnp[i][3] >= y) {
            btnf[i]()
        }
    }
})
...
function fr(x,y,xs,ys,xo,yo,r) {
    x = mod(x, 750)-750*(x>375)
    y = mod(y, 750)-750*(y>375)
    ctx.fillRect(x-(xs/2-xo)*Math.pow(2,r), y-(ys/2-yo)/Math.pow(2,r), xs*Math.pow(2, r), ys/Math.pow(2,r))
    ctx.fillRect(x-(xs/2-xo)*Math.pow(2,r)+750, y-(ys/2-yo)/Math.pow(2,r), xs*Math.pow(2, r), ys/Math.pow(2,r))
...
Mf={
    "nothing": (i) => {
        if (Math.random() < 0.004) {
            ...
            pos[i][4] += (Math.random()*2-1)*10
        }
    },
    "follow": (i) => {
        pos[i][3] += (pos[sqM[i][1]][1]-pos[i][1])/100
        pos[i][4] += (pos[sqM[i][1]][2]-pos[i][2])/100
    }
}
function a() {
    ...
    for (let i = 0; i < pos.length; i++) {
        if (Math.random() < 0.0001) {
            let Mi = Math.floor(Math.random()*(M.length-1))
            Mi += (Mi>=sqM[i][0])
            sqM[i] = [Mi]
            if (Mi == 1) {
                let n = Math.floor(Math.random()*(sq.length-1))
                n += (n>=i)
                sqM[i].push(n)
            }
        }
        ...
        pos[i][3] /= 1.1
        pos[i][4] /= 1.1
        pos[i][1] = mod(pos[i][1], 750)
        pos[i][2] = mod(pos[i][2], 750)
        let xr = Math.max(0, Math.min(10, Math.abs(pos[i][3])))
        let yr = Math.max(0, Math.min(10, Math.abs(pos[i][4])))
        let r = (xr*(xr>yr)-yr*(yr>xr))/10
        let ro = Math.round(Math.atan2(-pos[i][4], pos[i][3])/Math.PI*2+5)%4
        for (let j = 0; j < Math.floor(sq[pos[i][0]][ro].length/5); j++) {
            ctx.fillStyle = sq[pos[i][0]][ro][j*5]
            fr(pos[i][1], pos[i][2], sq[pos[i][0]][ro][j*5+3], sq[pos[i][0]][ro][j*5+4], sq[pos[i][0]][ro][j*5+1], sq[pos[i][0]][ro][j*5+2], -r)
        }
    }
